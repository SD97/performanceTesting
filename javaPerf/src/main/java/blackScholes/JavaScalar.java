...
public class JavaScalar {
    ...
    public double calculateBlackScholesSingleCycle(double[] spotPrices,double[] timeToMaturity,double[] strikePrice,
                                                   double[] interestRate,double[] volatility, int i)
    {
        ...
        double call = spotPrices[i] * CNDF(d1)
                - strikePrice[i]*Math.exp(-1.0 * rateScaled * timeToMaturity[i]) * CNDF(d2);
        return call;
    }
